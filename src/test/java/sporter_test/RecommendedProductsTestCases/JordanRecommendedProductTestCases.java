...
public class JordanRecommendedProductTestCases extends RecommendedProductTestCases {
    ...
    public void verifyAllRecommendedPopupComponentsIsExist() {
        ...
        WebElementsAssertion.validateTheElementIsDisplayed(jordanRecommendedProductPage.getProductImages(),webDriver);
        //Make sure the products prices are displayed
        WebElementsAssertion.validateTheElementIsDisplayed(jordanRecommendedProductPage.getProductPrice(),webDriver);
        //Make sure the add to cart buttons are displayed
        WebElementsAssertion.validateTheElementIsDisplayed(jordanRecommendedProductPage.getAddToCartBtn(),webDriver);
    ...
    public void verifyTheNameOfProductAddedToCatDisplayedInTheRecommendedPopup() {
        ...
        String productNameInPDP=DataHelperAndWait.getWebElementText(jordanProductDetailsPage.getProductName(),webDriver);
        DataHelperAndWait.clickOnElement(jordanProductDetailsPage.getAddToCartBtn(),webDriver);
        WebElementsAssertion.assertionWebElementContainsText(jordanRecommendedProductPage.getAddedProductToCartMsg(),webDriver,productNameInPDP);
    }
    ...
    public void verifyTheProductPriceInTheRecommendedPopupMatchesWithThePriceDisplaysInPdp() {
        ...
        String productPriceInRecommendedPopup = DataHelperAndWait.getWebElementText(jordanRecommendedProductPage.getProductPriceForFirstProduct(),webDriver);
        DataHelperAndWait.clickOnElement(jordanRecommendedProductPage.getAddToCartBtnForFirstProduct(),webDriver);
        String productPriceInPdp =DataHelperAndWait.getWebElementText(jordanProductDetailsPage.getFinalProductPrice(),webDriver);
    ...
    public void verifyClickingOnProductCardInTheRecommendedPopupWillDisplayThePdp() {
        ...
        JordanProductDetailsPage jordanProductDetailsPage = new JordanProductDetailsPage(webDriver);
        jordanRecommendedProductPage.displayTheRecommendedProductsPopUp();
        String productNameInRecommendedPopup = DataHelperAndWait.getWebElementText(jordanRecommendedProductPage.getFirstProductName(),webDriver);
        DataHelperAndWait.clickOnElement(jordanRecommendedProductPage.getFirstProductCard(),webDriver);
        Assert.assertEquals(productNameInRecommendedPopup, DataHelperAndWait.getWebElementText(jordanProductDetailsPage.getProductName(),webDriver), "The product name displayed in the Recommended pop-up is not match with the name in PDP ");
    }
}
