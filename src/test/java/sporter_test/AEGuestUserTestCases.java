...
public class AEGuestUserTestCases extends BaseTest {
    ...
    @Test(groups = {"2.04 Guest User Page","All Smoke Testing Result","1.1 Critical Severity"},description = "Make sure to access the Guest Checkout page from the cart page correctly", priority = 2)
    ...
    @Test(groups = {"2.04 Guest User Page","All Smoke Testing Result","1.1 Critical Severity"},description = "Make sure to access the Guest Checkout page from the cart pop up correctly", priority = 3)
    ...
    @Test(groups = {"2.04 Guest User Page","1.4  Low Severity"},description = "Make sure the title section appearing in the Shipping information screen is displayed correctly ", priority = 4)
...
//        this.verifyAbilityToAccessTheGuestCheckoutPageFormTheCartPageCorrectly();
    ...
    @Test(groups = {"2.04 Guest User Page","All Smoke Testing Result","1.4  Low Severity"},description = "Guest Checkout- Make sure the system fills the store country by default in the country field in the shipping information form", priority = 5)
    ...
    @Test(groups = {"2.04 Guest User Page","All Smoke Testing Result","1.2 High Severity"},description = "Make sure the Guest user cannot submit the shipping information without filling the required fields empty", priority = 6)
...
//        Assert.assertEquals(aeGuestUserPage.getAddressRequiredFieldMsg().getText(), "This is a required field.");
    ...
    @Test(groups = {"2.04 Guest User Page","All Smoke Testing Result","1.3 Medium Severity"},description = "Make sure the Guest user cannot submit the shipping information when the phone number length is small ", priority = 7)
...
//        aeGuestUserPage.clickOnGuestCheckoutBtn();
        Select select = new Select(aeGuestUserPage.getCityMenu());
        select.getOptions().get(0).click();
        aeGuestUserPage.fillInShippingInformationInputField(aeGuestUserPage.firstName,aeGuestUserPage.lastName,aeGuestUserPage.email, "2222", aeGuestUserPage.streetLineOne, aeGuestUserPage.streetLineTwo);
        DataHelperAndWait.scrollTo(aeGuestUserPage.getContinueBtn(),webDriver);
aeGuestUserPage.clickOnContinueBtn();
        Assert.assertEquals(aeGuestUserPage.getPhoneRequiredFieldMsg().getText(), "Text length does not satisfy specified text range.");
    }
    @Test(groups = {"2.04 Guest User Page","All Smoke Testing Result","1.3 Medium Severity"},description = "Make sure the Guest user cannot submit the shipping information with incorrect email format ", priority = 8)
...
//        aeGuestUserPage.clickOnGuestCheckoutBtn();
        //        aeGuestUserPage.clearShippingInformationFields();
        aeGuestUserPage.fillInShippingInformationInputField(aeGuestUserPage.firstName, aeGuestUserPage.lastName, "email@email", aeGuestUserPage.phone, aeGuestUserPage.streetLineOne, aeGuestUserPage.streetLineTwo);
        Select select = new Select(aeGuestUserPage.getCityMenu());
...
//        this.verifyAbilityToAccessTheGuestCheckoutPageFormTheCartPageCorrectly();
    ...
    @Test(groups = {"2.04 Guest User Page","All Smoke Testing Result","1.4  Low Severity"},description = "Guest Checkout- Make sure  ability to fill and navigate between the shipping information form using Tab key and keyboard shortcuts", priority = 12)
    public void verifyAbilityToFillAndNavigateBetweenTheShippingInformationFormUsingTabKeyAndKeyboardShortCut() {
        AEGuestUserPage aeGuestUserPage = new AEGuestUserPage(webDriver);
        Actions act = new Actions(webDriver);
                aeGuestUserPage.clearShippingInformationFields();
                //Address Name is removed based on ticket 9025
//        this.verifyAbilityToAccessTheGuestCheckoutPageFormTheCartPageCorrectly();
...
//        Assert.assertTrue(aeGuestUserPage.getShippingMethodSection().isDisplayed());
    ...
    public void verifyInabilityToSubmitTheShippingInformationFormWhenFillingFieldsWithTextLengthGreaterThan255Symbols() {
        ...
        this.accessShippingInformationScreen();
//        this.verifyAbilityToAccessTheGuestCheckoutPageFormTheCartPageCorrectly();
        aeGuestUserPage.fillInShippingInformationInputField(aeGuestUserPage.longLengthText,aeGuestUserPage.longLengthText, aeGuestUserPage.email, aeGuestUserPage.phone, aeGuestUserPage.longLengthText,aeGuestUserPage.longLengthText);
        Select select = new Select(aeGuestUserPage.getCityMenu());
        WebElement cityOption = select.getOptions().get(2);
        cityOption.click();
        DataHelperAndWait.scrollTo(aeGuestUserPage.getContinueBtn(),webDriver);
    ...
    @Test(groups = {"2.04 Guest User Page","All Smoke Testing Result","1.1 Critical Severity"},description = "Make sure the Guest user can filling the shipping information correctly", priority = 14)
...
DataHelperAndWait.waitForTime(1000);
    ...
    @Test(groups = {"2.04 Guest User Page","1.3 Medium Severity"},description = "Make sure ability to navigate to the home page by clicking on the sporter logo from the shipping information form for the Guest User ", priority = 16)
...
//        this.verifyAbilityToAccessTheGuestCheckoutPageFormTheCartPageCorrectly();
    ...
    @Test(groups = {"2.04 Guest User Page","1.3 Medium Severity"},description = "Guest Checkout- Make sure ability to navigate to the shipping information directly from the URL", priority = 17)
    ...
    @Test(groups = {"2.04 Guest User Page","1.1 Critical Severity"},description = "Make sure the Guest user can filling the shipping information correctly when switching to Arabic version", priority = 18)
    public void verifyTheGuestUserCanFillTheShippingInformationWhenSwitchingToArabicCorrectly() {
        ...
        this.accessShippingInformationScreen();
        //        aeGuestUserPage.clearShippingInformationFields();
        aeGuestUserPage.fillInShippingInformationInputField(aeGuestUserPage.firstNameAr, aeGuestUserPage.lastNameAr, aeGuestUserPage.email, aeGuestUserPage.phone,  aeGuestUserPage.streetLineOneAr, aeGuestUserPage.streetLineTwoAr);
        Select select = new Select(aeGuestUserPage.getCityMenu());
        WebElement cityOption = select.getOptions().get(2);
        cityOption.click();
        DataHelperAndWait.scrollTo(aeGuestUserPage.getContinueBtn(),webDriver);
aeGuestUserPage.clickOnContinueBtn();
    ...
    //There's a bug here
    @Test(groups = {"2.04 Guest User Page","1.4  Low Severity"}, description = "Guest User- Make sure the validation message appears when submiting the shipping information without filling the city field", priority = 19)
