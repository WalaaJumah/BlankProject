...
public class KSARecommendedProductTestCases extends RecommendedProductTestCases {
    ...
    public void verifyAllRecommendedPopupComponentsIsExist() {
        ...
        WebElementsAssertion.validateTheElementIsDisplayed(kSARecommendedProductPage.getProductImages(),webDriver);
        //Make sure the products prices are displayed
        WebElementsAssertion.validateTheElementIsDisplayed(kSARecommendedProductPage.getProductPrice(),webDriver);
        //Make sure the add to cart buttons are displayed
        WebElementsAssertion.validateTheElementIsDisplayed(kSARecommendedProductPage.getAddToCartBtn(),webDriver);
    ...
    public void verifyTheNameOfProductAddedToCatDisplayedInTheRecommendedPopup() {
        ...
        String productNameInPDP=DataHelperAndWait.getWebElementText(kSAProductDetailsPage.getProductName(),webDriver);
        DataHelperAndWait.clickOnElement(kSAProductDetailsPage.getAddToCartBtn(),webDriver);
        WebElementsAssertion.assertionWebElementContainsText(kSARecommendedProductPage.getAddedProductToCartMsg(),webDriver,productNameInPDP);
    }
    ...
    public void verifyTheProductPriceInTheRecommendedPopupMatchesWithThePriceDisplaysInPdp() {
        ...
        String productPriceInRecommendedPopup = DataHelperAndWait.getWebElementText(kSARecommendedProductPage.getProductPriceForFirstProduct(),webDriver);
        DataHelperAndWait.clickOnElement(kSARecommendedProductPage.getAddToCartBtnForFirstProduct(),webDriver);
        String productPriceInPdp =DataHelperAndWait.getWebElementText(kSAProductDetailsPage.getFinalProductPrice(),webDriver);
    ...
    public void verifyClickingOnProductCardInTheRecommendedPopupWillDisplayThePdp() {
        ...
        KSAProductDetailsPage kSAProductDetailsPage = new KSAProductDetailsPage(webDriver);
        kSARecommendedProductPage.displayTheRecommendedProductsPopUp();
        String productNameInRecommendedPopup = DataHelperAndWait.getWebElementText(kSARecommendedProductPage.getFirstProductName(),webDriver);
        DataHelperAndWait.clickOnElement(kSARecommendedProductPage.getFirstProductCard(),webDriver);
        Assert.assertEquals(productNameInRecommendedPopup, DataHelperAndWait.getWebElementText(kSAProductDetailsPage.getProductName(),webDriver), "The product name displayed in the Recommended pop-up is not match with the name in PDP ");
    }
}
